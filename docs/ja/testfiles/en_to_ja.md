このコメントは、翻訳されないようにするためのコードブロックです。
コロンの右側のエントリは次のようになります。
- dir docs/testfiles/en/ では英語で記述します。
- dir docs/testfiles/ja/ では日本語で記述します。

コロンの左側の内容は翻訳しないでください。
「should be」の右側のテキストは、docs/testfiles/ja/ dir の翻訳されたテキストと一致する必要があります。
```
