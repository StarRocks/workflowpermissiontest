...
TPC-H 是由事务处理性能委员会（TPC）开发的决策支持基准测试。它由一套面向业务的即席查询和并发数据修改组成。TPC-H 可用于基于真实生产环境构建模型，以模拟销售系统的数据仓库。本测试使用八张表，数据量从 1 GB 到 3 TB 不等。共测试 22 个查询，主要性能指标为每个查询的响应时间，即从提交查询到返回结果之间的持续时间。

## 1. 测试结论

在 TPC-H 100G 规模数据集上进行了对比测试，共 22 个查询，结果如下：

![TPCH 100G 测试结果](../_assets/tpch.png)

StarRocks 测试了两种方法：本地存储查询和 Hive 表查询。StarRocks Hive 表与 Trino 查询相同的数据。数据以 ORC 格式存储，并以 zlib 格式压缩。

StarRocks 查询本地存储数据的延迟为 21s，查询 Hive 外部表的延迟为 92s，Trino 查询 Hive 外部表的延迟为 187s。
...
| 机器 | 4 台云主机 |
| -------- | ----------------------------------------------------- |
| CPU | 16核 Intel(R) Xeon(R) Platinum 8269CY CPU @ 2.50GHz |
| 内存 | 64 GB |
| 网络带宽 | 5 Gbits/s |
| 磁盘 | ESSD 云盘 |
...
StarRocks 和 Trino 部署在相同配置的机器上。StarRocks 部署了 1 个 FE 和 3 个 BE。Trino 部署了 1 个 Coordinator 和 3 个 Worker。
...
:::note
查询结果的单位为 ms，越低越好。
所有查询预热 1 次后执行 3 次，取平均值作为结果。
:::

| 查询 | StarRocks-native-3.0 | StarRocks-3.0-Hive 外部表 | Trino-419 |
| ---- | -------------------- | ---------------------------- | --------- |
| Q1 | 1540 | 5660 | 8811 |
| Q2 | 100 | 1593 | 3009 |
| Q3 | 700 | 5286 | 7891 |
...
下载 tpch-poc 工具包，为 TPC-H 标准测试集 `scale factor=100` 生成数据。
...
修改配置文件 `conf/starrocks.conf`，指定集群地址（host 和 port），然后执行建表操作。
...
### 4.2 使用 StarRocks 查询 Hive 外部表

#### 4.2.1 创建表结构

在 Hive 中创建外部表。外部表存储格式为 ORC，压缩格式为 zlib。详细建表语句请参见[5.3](#53-hive-external-table-creation-orc-storage-format)。此 Hive 外部表同时用于 StarRocks 和 Trino 查询。

http://localhost:3000/docs/benchmarking/TPC-H_Benchmarking/

#### 4.2.2 导入数据

将步骤 4.1.1 中生成的 TPC-H CSV 原始数据上传至 HDFS 指定路径（本文使用路径 `/user/tmp/csv/`），然后在 Hive 中创建外部表。详细建表语句请参见[5.4](#54-hive-external-table-creation-csv-storage-format)。该 Hive 外部表的存储格式为 CSV，存储路径为 `/user/tmp/csv/`，即上传 CSV 原始数据的路径。

使用 `INSERT INTO` 将 CSV 格式外部表的数据导入 ORC 格式的外部表中，从而以 ORC 存储格式和 zlib 压缩格式获取数据。导入命令如下：
...
StarRocks 使用 Catalog 功能查询 Hive 表数据。详细操作请参见 [Hive catalog](../data_source/catalog/hive_catalog.md)。
