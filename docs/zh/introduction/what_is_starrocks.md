...
StarRocks 是一款新一代极速全并行处理（MPP）数据库，旨在让企业能够轻松进行实时分析。它的设计目标是以极快的速度支持大规模查询。
...
StarRocks 兼容 MySQL 协议和标准 SQL，开箱即用地支持所有主流的商业智能（BI）工具，如 Tableau 和 Power BI。StarRocks 不依赖任何外部组件。它是一个集成的数据分析平台，具有高可扩展性、高可用性和简化的管理和维护。
...
StarRocks 利用高性能的数据分布、灵活的索引和智能物化视图来促进面向用户的高并发分析：
