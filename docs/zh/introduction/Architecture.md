...
StarRocks 支持 shared nothing（每个 BE 在其本地存储上都有一部分数据）和 shared-data（对象存储或 HDFS 上的所有数据，每个 CN 仅在本地存储上具有缓存）。您可以根据自己的需求决定数据的存储位置。
...
作为一种典型的大规模并行处理 (MPP) 数据库，StarRocks 支持 shared nothing 架构。在此架构中，BE 负责数据存储和计算。直接访问 BE 模式上的本地数据可以进行本地计算，从而避免了数据传输和数据复制，并提供了超快的查询和分析性能。此架构支持多副本数据存储，从而增强了集群处理高并发查询的能力并确保了数据可靠性。它非常适合追求最佳查询性能的场景。
...
FE 负责元数据管理、客户端连接管理、查询计划和查询调度。每个 FE 使用 BDB JE (Berkeley DB Java Edition) 在其内存中存储和维护元数据的完整副本，从而确保所有 FE 上的一致服务。FE 可以充当 Leader、Follower 和 Observer。如果 Leader 节点崩溃，则 Follower 会根据 Raft 协议选举出一个 Leader。
...
- SQL 执行：FE 根据查询的语义将每个 SQL 查询解析为逻辑执行计划，然后将逻辑计划转换为可以在 BE 上执行的物理执行计划。存储目标数据的 BE 执行查询。这样就无需数据传输和复制，从而实现了高查询性能。
