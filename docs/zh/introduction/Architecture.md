...
StarRocks 与 MySQL 协议兼容，并支持标准 SQL。用户可以轻松地从 MySQL 客户端连接到 StarRocks，以获得即时且有价值的见解。
...
作为一种典型的大规模并行处理 (MPP) 数据库，StarRocks 支持 shared nothing 架构。在此架构中，BE 负责数据存储和计算。直接访问 BE 模式下的本地数据可以进行本地计算，从而避免了数据传输和数据复制，并提供了超快的查询和分析性能。此架构支持多副本数据存储，从而增强了集群处理高并发查询的能力并确保了数据可靠性。它非常适合追求最佳查询性能的场景。
...
FE 负责元数据管理、客户端连接管理、查询计划和查询调度。每个 FE 都使用 BDB JE (Berkeley DB Java Edition) 来存储和维护其内存中元数据的完整副本，从而确保所有 FE 上的一致服务。FE 可以充当 leader、follower 和 observer。如果 leader 节点崩溃，则 follower 将根据 Raft 协议选举 leader。
