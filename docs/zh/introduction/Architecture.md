...
StarRocks 具有简单的架构。整个系统仅由两种类型的组件组成：前端和后端。前端节点称为 **FE**。后端节点有两种类型，**BE** 和 **CN** (计算节点)。当使用本地存储来存储数据时，则部署 BE。当数据存储在对象存储或 HDFS 上时，则部署 CN。StarRocks 不依赖任何外部组件，从而简化了部署和维护。节点可以水平扩展，而不会导致服务中断。此外，StarRocks 具有元数据和服务数据的副本机制，从而提高了数据可靠性，并有效地防止了单点故障 (SPOF)。
...
作为一种典型的海量并行处理 (MPP) 数据库，StarRocks 支持 shared-nothing 架构。在此架构中，BE 负责数据存储和计算。直接访问 BE 模式下的本地数据可以进行本地计算，从而避免了数据传输和数据复制，并提供了超快的查询和分析性能。此架构支持多副本数据存储，从而增强了集群处理高并发查询的能力并确保数据可靠性。它非常适合追求最佳查询性能的场景。
...
FE 负责元数据管理、客户端连接管理、查询规划和查询调度。每个 FE 使用 BDB JE (Berkeley DB Java Edition) 在其内存中存储和维护元数据的完整副本，从而确保所有 FE 上的一致服务。FE 可以充当 Leader、Follower 和 Observer。如果 Leader 节点崩溃，则 Follower 会根据 Raft 协议选举出一个 Leader。
...
- SQL 执行：FE 根据查询的语义将每个 SQL 查询解析为逻辑执行计划，然后将逻辑计划转换为可以在 BE 上执行的物理执行计划。存储目标数据的 BE 执行查询。这样就不需要数据传输和复制，从而实现高查询性能。
...
StarRocks shared-data 集群将数据存储和计算分离，从而允许两者独立扩展，从而降低了成本并提高了弹性。但是，此架构可能会影响查询性能。
