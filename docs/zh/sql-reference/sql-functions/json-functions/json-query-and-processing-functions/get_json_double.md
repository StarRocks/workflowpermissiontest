...
该函数用于解析 JSON 字符串，并从指定的路径中获取浮点数值。`json_path` 必须以 `$` 开头，并使用 `.` 作为路径分隔符。

:::tip
所有 JSON 函数和操作符均在导航栏和 [概览页面](../overview-of-json-functions-and-operators.md) 中列出。

使用 [generated columns](../../../sql-statements/generated_columns.md) 加速查询。
:::

如果路径中包含 `.`，则可以使用 `"` 和 `"` 将其括起来。
